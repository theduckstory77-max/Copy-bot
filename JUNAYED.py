...
# আপনার কোডের সব লাইব্রেরি এখানে লিস্ট করা হলো
libraries = [
    ('urllib3', None),
    ('PyJWT', 'jwt'),
    ('requests', None),
    ...
    ('python-cfonts', 'cfonts'), # এবারের 'cfonts' এরর এখান থেকে ঠিক হবে
    ('protobuf-decoder', 'protobuf_decoder'), # এটিও আপনার কোডে আছে
]

for package, import_name in libraries:
    install_and_import(package, import_name)

print("All libraries installed! Server is starting...")
# --- এরপর থেকে আপনার বাকি কোড শুরু হবে ---
import os
import sys
import time
import json
...
ADMIN_UID = "2455328236"
GUEST_UID = "4535463540"
GUEST_PASSWORD = "F21470DFBA8BB9153BD8BC844A64CC00CE5A31664B3FDB56FFB4F35164A6A68E"
region = 'IN'
...
async def Junayed(client_id, key, iv):
    banner_text = f""" 
.
.
.
...
[CCFFCC]━━━━━━━━━━━━━━━━━━━━━━━━━━
[B][C][81DACA]P O W E R E D   B Y  J N A Y E D
[FFFF00]━━━━━━━━━━━━━━━━━━━━━━━━━━
[B][C][00FF00]F O L L O W   M E   O N   T I K T O K
[B][C][FFFFFF]@JUNAYED98
[00008B]━━━━━━━━━━━━━━━━━━━━━━━━━━
...
[B][C][FF0000]F U C K   Y O U
[CCFFCC]━━━━━━━━━━━━━━━━━━━━━━━━━━
[B][C][81DACA]P O W E R E D   B Y   J U N A Y E D
[FFFF00]━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async def convert_kyro_to_your_system(target_uid, chat_id, key, iv, nickname="JUNAYED", title_id=904990072):
    try:
        fields = {
            1: 1,
            2: {
...
async def send_kyro_title_adapted(chat_id, key, iv, target_uid, nickname="JUNAYED", title_id=905190079):
    try:
...
async def send_all_titles_once(chat_id, key, iv, target_uid, whisper_writer, nickname="JUNAYED"):
    titles = [
...
async def handle_title_final(inPuTMsG, uid, chat_id, key, iv, region, chat_type=0):
    ...
    try:
        if not whisper_writer:
            return
        title_packet = await convert_kyro_to_your_system(target_uid, chat_id, key, iv, "JUNAYED", selected_title)
        if title_packet and whisper_writer:
            whisper_writer.write(title_packet)
            await whisper_writer.drain()
    except Exception:
...
async def send_sticker(target_uid, chat_id, key, iv, nickname="JUNAYEDXBOT"):
    try:
            ...
            while True:
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        insquad = True
                        try:
                            welcome_message = """[B][C][FFFFFF]Welcome to JUNAYED Bot
...
[B][C][FFFFFF]Telegram : JUNAYED
[B][C][FFFFFF]Contact : @JUNAYED
...
[B][C][FFFFFF]Status: Online & Active"""
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = """[B][C][FFFFFF]Welcome to JUNAYED Bot
...
[C][B][FFFFFF]JUNAYED CODEX

[B][C][FFFFFF]Telegram : @Junayed
[B][C][FFFFFF]Contact : @h4ker6t9
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 3:
                                ...
                                await dl_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                try:
                                    success = await Room_Spam(target_uid, room_id, "JUNAYED CODEX", key, iv)
                                    if success:
                        ...
                        if inPuTMsG.strip().startswith('/sticker'):
                            packet = await send_sticker(uid, chat_id, key, iv, nickname="JUNAYEDXBOT")
                            if packet:
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                await dl_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().lower() == "Junayed bhai mera uid do":
...
async def MaiiiinE():
    ...
    Uid, Pw = '4535463540', 'F21470DFBA8BB9153BD8BC844A64CC00CE5A31664B3FDB56FFB4F35164A6A68E'
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 Junayed BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print(render('Junayed', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 JUNAYED BOT - ONLINE")
    print("=" * 50)
    print(f"🔹 UID: {TarGeT}")
